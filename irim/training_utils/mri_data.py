...
class SliceData(Dataset):
    ...
    def __init__(self, root, transform, challenge, sample_rate=1, n_slices=1, use_rss=False, bbox_root=None):
        """
    ...
    def __getitem__(self, i):
        ...
        with h5py.File(fname, 'r') as data:
            ...
            item = self.transform(kspace, target, attributes, fname.name, slice, n_slices=self.n_slices)
            if self.bbox_root is not None:
                seg_fname = self.bbox_root+fname.name[:-2]+'cii'
                try:
                    seg = h5py.File(seg_fname, 'r')['mask'][()]
                    seg = np.flip(seg, 0)
                    seg = seg.transpose(2, 0, 1, 3)
                    seg = seg[slice].copy()
                    seg = torch.tensor(seg).to(torch.float32)
                except Exception as e:
                    seg = torch.zeros((320, 320, 11)).to(torch.float32)
                item = item, seg
            return item 
